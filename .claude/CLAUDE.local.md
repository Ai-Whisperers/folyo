...
### NEVER Run Parallel Extractions

**RULE: Always run social media extractions ONE AT A TIME, sequentially.**

Running multiple extractions in parallel will trigger security measures on Facebook/Instagram:
- Facebook detects multiple simultaneous logins from the same account
- Instagram flags suspicious activity from parallel browser sessions
- Results in "Login verification failed" errors for ALL extractions
- May temporarily lock the account

**Correct approach:**
```bash
# Sequential extraction (CORRECT)
python extract.py --account company1 --platforms facebook ...
# Wait for completion
python extract.py --account company2 --platforms facebook ...
# Wait for completion
python extract.py --account company3 --platforms facebook ...
```

**Wrong approach:**
```bash
# Parallel extraction (WRONG - TRIGGERS SECURITY)
python extract.py --account company1 ... &
python extract.py --account company2 ... &
python extract.py --account company3 ... &
```

**For batch scripts:** Use sequential execution, not parallel. Add delays between extractions if needed.
